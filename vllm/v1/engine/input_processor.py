...
class InputProcessor:
    ...
    def _validate_watermarking_params(self, params: SamplingParams) -> None:
        ...
        if params.seed is not None:
            raise VLLMValidationError(
                "Explicit request seeds are not supported with watermarking. "
                "Remove seed or set watermarking=False.",
                parameter="seed",
                value=params.seed,
            )
        if params.n > 1:
            raise VLLMValidationError(
                "Parallel sampling with n > 1 is not supported with watermarking. "
                "Set n=1 or watermarking=False.",
                parameter="n",
                value=params.n,
            )
