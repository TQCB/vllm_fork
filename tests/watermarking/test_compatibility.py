...
@pytest.mark.parametrize(
    ...
    [
        ...
        (SamplingParams(seed=42), "Explicit request seeds"),
        (SamplingParams(n=2), "Parallel sampling"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SamplingParams(seed=42, watermarking=False),
        SamplingParams(n=2, watermarking=False),
...
def test_direct_engine_request_is_rejected_before_id_mutation():
    ...
    request = _engine_core_request(SamplingParams(n=2))

    with pytest.raises(VLLMValidationError, match="Parallel sampling"):
        engine.add_request(
            request.request_id,
            request,
